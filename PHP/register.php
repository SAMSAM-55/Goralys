...
if (empty($email) || empty($password) || empty($user_name) || empty($user_id)) {
    echo "<script type='text/javascript'>
        window.location.href = window.location.origin + '$folder' + '/register_page.php';
...
$create_account_stmt = $conn->prepare("INSERT INTO saje5795_goralys.users_temp (user_id, full_name, email, password_hash, role, verification_token) VALUES(?, ?, ?, ?, ?, ?)");
...
$create_account_stmt->bind_param("s"*6, $user_id, $user_name, $email, $hashed_password, $role, $token);
