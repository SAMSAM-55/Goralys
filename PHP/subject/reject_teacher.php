...
if (!$student_id || !$topic_id || !$subject) {
    GoralysUtility::showToast(
        'error',
        "Sujet",
        "Une erreur est survenue lors du rejet du sujet. Veuillez réessayer ultérieurement.",
        js: true
    );
...
$query = "UPDATE saje5795_goralys.student_topics SET subject_status = 2, last_rejected = ?
...
if (!$stmt->execute()) {
    GoralysUtility::showToast(
        'error',
        "Sujet",
        "Une erreur interne est survenue lors du rejet du sujet.Veuillez réessayer ultérieurement.",
        js: true
    );
